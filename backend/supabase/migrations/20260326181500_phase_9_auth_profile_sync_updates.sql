...
select
  au.id,
  au.email,
  coalesce(nullif(trim(au.raw_user_meta_data ->> 'first_name'), ''), 'User'),
  coalesce(nullif(trim(au.raw_user_meta_data ->> 'last_name'), ''), 'Account'),
  nullif(trim(au.raw_user_meta_data ->> 'job_title'), ''),
  coalesce((au.raw_app_meta_data ->> 'app_role')::public.app_role, 'employee'),
  true
from auth.users au
on conflict (id) do update
set
