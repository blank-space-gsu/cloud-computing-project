  ...
  const demoHint = el('div', { style: 'margin-top:20px;padding:14px;background:var(--color-primary-bg);border-radius:var(--radius-sm);font-size:0.82rem;color:var(--color-text-secondary)' },
    ...
    el('div', { className: 'btn-group', style: 'margin-top:12px' },
      el('button', {
        ...
        type: 'button',
        onClick: () => { form.querySelector('#email').value = 'manager.demo@cloudcomputing.local'; }
      }, 'Use Manager Demo'),
      el('button', {
        className: 'btn btn-outline btn-sm',
        type: 'button',
        onClick: () => { form.querySelector('#email').value = 'employee.one@cloudcomputing.local'; }
      }, 'Use Employee Demo')
    )
  );

  const card = el('div', { className: 'login-card' },
    el('button', { className: 'login-back-link', type: 'button', onClick: () => navigate('#/') }, '← Back to home'),
